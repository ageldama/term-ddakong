...
BOOL
dll_plugin_load (dll_plugin_context_t *p_plugin_ctx,
                 const char *sz_dll_filename)
{
  ...
  void *fun = dlsym (p_plugin_ctx->p_dll, DDAKONG_PLUGIN_ENTRY_NAME);
  if (NULL == fun)
    {
      perror (dlerror ());
...
void
dll_plugin_unload (dll_plugin_context_t *p_plugin_ctx)
{
  assert (p_plugin_ctx != NULL);
  assert (p_plugin_ctx->p_dll != NULL);

  /* deinit */
  ddakong_plugin_deinit_fn_t deinit_fn = (ddakong_plugin_deinit_fn_t)dlsym (
      p_plugin_ctx->p_dll, DDAKONG_PLUGIN_DEINIT_NAME);
  if (NULL == deinit_fn)
    {
      perror (dlerror ());
    }
...
BOOL
dll_plugin_load (dll_plugin_context_t *p_plugin_ctx UNUSED,
...
void
dll_plugin_unload (dll_plugin_context_t *p_plugin_ctx UNUSED)
{
}
